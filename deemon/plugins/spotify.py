...
from deemon.plugins import Plugin
...
class Spotify(Plugin):
    def __init__(self, configFolder=None):
        ...
        if configFolder is None:
            deemix_config_dir = getConfigFolder()
            self.configFolder = Path(deemix_config_dir) / 'spotify'
        else:
            self.configFolder = Path(configFolder)
    ...
    def parseLink(cls, link):
        ...
        if not 'spotify' in link: return (link, link_type, link_id)
    ...
    def generateAlbumItem(self, dz, link_id, bitrate):
        ...
        if link_id in cache['albums']:
            ...
            logger.debug(f"Using cached album data")
        else:
            logger.debug(f"Fetching album data from Spotify")
            cachedAlbum = self.getAlbum(link_id)
            cache['albums'][link_id] = cachedAlbum
            self.saveCache(cache)

        logger.info(f"Spotify album: {cachedAlbum['data']['artist']} - {cachedAlbum['data']['title']}, UPC: {cachedAlbum['upc']}")
        
        try: 
            logger.debug(f"Trying to find album by UPC: {cachedAlbum['upc']}")
            deezerAlbum = generateAlbumItem(dz, f"upc:{cachedAlbum['upc']}", bitrate)
            logger.info(f"Successfully found album by UPC")
            return deezerAlbum
        except GenerationError as e:
            logger.warning(f"UPC search failed, trying fallback search: {e}")
            if self.settings['fallbackSearch']:
                searchQuery = f"{cachedAlbum['data']['artist']} {cachedAlbum['data']['title']}"
                logger.info(f"Searching for album: {searchQuery}")
                try:
                    albumData = dz.api.search_album(searchQuery)
                    logger.debug(f"Search results: {albumData}")
                    if albumData and len(albumData) > 0:
                        bestMatch = albumData[0]
                        logger.info(f"Found Deezer album: {bestMatch['id']} - {bestMatch['title']} by {bestMatch['artist']['name']}")
                        cachedAlbum['id'] = bestMatch['id']
                        cache['albums'][link_id] = cachedAlbum
                        self.saveCache(cache)
                        deezerAlbum = generateAlbumItem(dz, cachedAlbum['id'], bitrate)
                        logger.info(f"Successfully generated album item from search result")
                        return deezerAlbum
                    else:
                        logger.warning(f"No search results found for album: {searchQuery}")
                except Exception as searchError:
                    logger.error(f"Error during album search: {searchError}")
    ...
    def getTrack(self, track_id, spotifyTrack=None, albumName=None):
        if not self.enabled: raise Exception("Spotify plugin not enabled")
        cachedTrack = {
            'isrc': None,
            'data': None
        ...
        if not trackAlbumName and 'album' in spotifyTrack:
            trackAlbumName = spotifyTrack['album']['name']
        
        cachedTrack['data'] = {
            'title': spotifyTrack['name'],
            'artist': spotifyTrack['artists'][0]['name'],
            'album': trackAlbumName or 'Unknown Album'
        }
        return cachedTrack
    ...
    def convertTrack(self, dz, downloadObject, track, pos, conversion, cache, listener):
        ...
        try:
            if downloadObject.isCanceled: return None
            trackAPI = None
            cachedTrack = None
            ...
            if track['id'] in cache['tracks']:
                ...
                albumName = downloadObject.collection.get('albumAPI', {}).get('title')
                cachedTrack = self.getTrack(track['id'], track, albumName)
                cache['tracks'][track['id']] = cachedTrack
                self.saveCache(cache)

            logger.info(f"Converting track {pos+1}/{downloadObject.size}: {cachedTrack['data']['artist']} - {cachedTrack['data']['title']}, ISRC: {cachedTrack.get('isrc', 'None')}")
            
            if 'isrc' in cachedTrack and cachedTrack['isrc']:
                try:
                    trackAPI = dz.api.get_track_by_ISRC(cachedTrack['isrc'])
                    if 'id' not in trackAPI or 'title' not in trackAPI: 
                        trackAPI = None
                        logger.debug(f"ISRC found but invalid track data")
                    else:
                        logger.info(f"Found track via ISRC: {trackAPI['id']} - {trackAPI['title']}")
                except DataException as e:
                    logger.debug(f"ISRC search failed: {e}")
                    trackAPI = None
            if self.settings['fallbackSearch'] and not trackAPI:
                logger.debug(f"Trying fallback search for track")
                if 'id' not in cachedTrack or cachedTrack['id'] == "0":
                    try:
                        trackID = dz.api.get_track_id_from_metadata(
                            cachedTrack['data']['artist'],
                        ...
                        if trackID != "0":
                            ...
                            cache['tracks'][track['id']] = cachedTrack
                            self.saveCache(cache)
                            logger.info(f"Found track via metadata search: {trackID}")
                    except Exception as metadataError:
                        logger.error(f"Metadata search error: {metadataError}")

                if cachedTrack.get('id', "0") != "0":
                    try:
                        trackAPI = dz.api.get_track(cachedTrack['id'])
                        logger.info(f"Got track via ID: {cachedTrack['id']}")
                    except Exception as idError:
                        logger.error(f"Error getting track by ID: {idError}")
                        trackAPI = None

            if not trackAPI:
                logger.warning(f"No track found for: {cachedTrack['data']['artist']} - {cachedTrack['data']['title']}")
                trackAPI = {
                    'id': "0",
                    'title': cachedTrack['data'].get('title', 'Unknown Track'),
                    'duration': track.get('duration_ms', 0) // 1000 if track.get('duration_ms') else 0,
                    'md5_origin': 0,
                    'media_version': 0,
                    'filesizes': {},
                    'album': {
                        'title': cachedTrack['data'].get('album', 'Unknown Album'),
                        'md5_image': ""
                    },
                    'artist': {
                        'id': 0,
                        'name': cachedTrack['data'].get('artist', 'Unknown Artist')
                    }
                }
            ...
            logger.error(f"Unexpected error converting track: {e}")
            return None
            return None
    ...
    def convert(self, dz, downloadObject, settings, listener=None):
        ...
        logger.info(f"Starting conversion of {len(downloadObject.conversion_data)} tracks")
        ...
        with ThreadPoolExecutor(settings['queueConcurrency']) as executor:
            for pos, track in enumerate(downloadObject.conversion_data, start=0):
                try:
                    result = executor.submit(self.convertTrack,
                        dz, downloadObject,
                        track, pos,
                        conversion,
                        cache, listener
                    ).result()
                    collection[pos] = result
                except Exception as e:
                    logger.error(f"Error converting track at position {pos}: {e}")
                    collection[pos] = None

        successful_conversions = [track for track in collection if track and track.get('id') != "0"]
        logger.info(f"Successfully converted {len(successful_conversions)} out of {len(collection)} tracks")
        
        downloadObject.collection['tracks'] = successful_conversions
        downloadObject.size = len(successful_conversions)
